...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Suvereno client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Suvereno"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Suvereno"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
