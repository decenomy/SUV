...
const char * const PIVX_CONF_FILENAME = "suvereno.conf";
const char * const PIVX_PID_FILENAME = "suvereno.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";
const char * const PIVX_ACTIVE_MASTERNODE_CONF_FILENAME = "activemasternode.conf";


// Suvereno only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\suvereno
// Windows >= Vista: C:\Users\Username\AppData\Roaming\suvereno
// Mac: ~/Library/Application Support/suvereno
// Unix: ~/.suvereno
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Suvereno";
#else
    fs::path pathRet;
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty suvereno.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override suvereno.conf
