...
{
    if (nHeight > 2700000) return 100000 * COIN;
    if (nHeight > 2600000) return  80000 * COIN;
    if (nHeight > 2500000) return  60000 * COIN;
    if (nHeight > 2400000) return  50000 * COIN;
    if (nHeight > 2300000) return  40000 * COIN;
    if (nHeight > 2200000) return  30000 * COIN;
    if (nHeight > 2100000) return  20000 * COIN;
    if (nHeight > 2012000) return   8000 * COIN;
    if (nHeight >  806400) return   6000 * COIN;
    if (nHeight >  403200) return   4500 * COIN;
...
{
    if (nHeight > 2012000) return CRewards::GetBlockValue(nHeight) * 65 / 100;
    if (nHeight >  403200) return CRewards::GetBlockValue(nHeight) * 90 / 100;
    if (nHeight >  201600) return CRewards::GetBlockValue(nHeight) * 85 / 100;
    if (nHeight >     224) return CRewards::GetBlockValue(nHeight) * 80 / 100;
    return 0;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 SUV tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                   // block for 1000 SUV tx -> 1 confirmation
