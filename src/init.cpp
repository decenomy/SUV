...
/** Sanity checks
 *  Ensure that Suvereno is running in a usable environment with all
...
#endif
    LogPrintf("Suvereno version %s (%s)\n", version_string, CLIENT_DATE);
}
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. Suvereno is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single Suvereno process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. Suvereno is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // Suvereno: load previous sessions sporks if we have them.
