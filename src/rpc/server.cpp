...
UniValue stop(const JSONRPCRequest& jsonRequest)
{
    ...
    if (jsonRequest.fHelp || jsonRequest.params.size() > 1)
        throw std::runtime_error(
            "stop\n"
            "\nStop Suvereno server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    return "Suvereno server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* SUV features */
        {"suvereno", "listmasternodes", &listmasternodes, true },
        {"suvereno", "getmasternodecount", &getmasternodecount, true },
        {"suvereno", "createmasternodebroadcast", &createmasternodebroadcast, true },
        {"suvereno", "decodemasternodebroadcast", &decodemasternodebroadcast, true },
        {"suvereno", "relaymasternodebroadcast", &relaymasternodebroadcast, true },
        {"suvereno", "masternodecurrent", &masternodecurrent, true },
        {"suvereno", "startmasternode", &startmasternode, true },
        {"suvereno", "reloadmasternodeconfig", &reloadmasternodeconfig, true },
        {"suvereno", "createmasternodekey", &createmasternodekey, true },
        {"suvereno", "getmasternodeoutputs", &getmasternodeoutputs, true },
...
{
    return "> suvereno-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(std::string methodname, std::string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:18977/\n";
}
